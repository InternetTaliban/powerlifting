...
export function BottomNav() {
  ...
  const btn = (k: NavKey, revealed: boolean) => {
    ...
    const cls = ['bottom-nav-btn', k === 'today' && 'nav-accent', revealed && 'nav-revealed'].filter(Boolean).join(' ');
  ...
  return (
    <nav id="bottomNav" className="bottom-nav" aria-label="Views">
      ...
      {order.filter((k) => cfg.layout[k] === 'bottom' && !suppressed(k)).map((k) => btn(k, false))}
      {anyBottom && reveal
        && order.filter((k) => cfg.layout[k] === 'hidden' && !suppressed(k)).map((k) => btn(k, true))}
